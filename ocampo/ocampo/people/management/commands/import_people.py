...
from ocampo.people.models import Place, Occupation
...
def map_csv(csvfile, headers=True):
    ...
    for row in reader:
        ...
        people.birthdate = row['Birthdate'] if row['Birthdate'] else None
        people.deathdate = row['Deathdate'] if row['Deathdate'] else None
        people.notes = row['Notes']
        people.sex = row['Sex']
        people.viaf_id = row['Viaf_id'] if row['Viaf_id'] else None
        ...
        birthplace, created = Place.objects.get_or_create(name=row['Birthplace'])

        people.birthplace = birthplace

        deathplace, create = Place.objects.get_or_create(name=row['Deathplace'])

        people.deathplace = deathplace
        ...
        for occupation in occupations:
            print(occupation)
            occupation_obj = Occupation.objects.get(name=occupation.strip())
            people.occupations.add(occupation_obj)
