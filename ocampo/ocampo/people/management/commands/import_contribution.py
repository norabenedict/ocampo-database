...
from ocampo.people.models import Contribution
...
from ocampo.people.models import Issue
...
def map_csv(csvfile, headers=True):
    ...
    for row in reader:
        ...
        contribution.title = row['Title']
        contribution.start_page = row['Start_Page']
        contribution.end_page = row['End_Page']
        contribution.contribution_type = row['Contribution_type']
        contribution.contribution_type_choice = row['Contribution_type_choice']
        contribution.notes = row['Notes']
        ...
        issue, created = Issue.objects.get_or_create(year=row['Year'], number=row['Number'], date=row['Date'])
